...
def wos_walk_altered_g(velocity, gradient, original, radius_g, dim, distance_field, min_corner_field, goal, start, overall_domain, min_corner, curr_rec):
    ...
    if dist_original < dist_b:
        ...
        point *= (dist_b/2)/((dist_original+1)*1.15)
...
def WoS_altered_g(velocity, gradient, max_heat, distance_field, min_corner_field, goal, start, radius, n_walks, overall_domain, min_corner):
    dim = len(start)
    final_path = []
    radius_g = distance_field[tuple(np.round(goal-min_corner_field).astype(int))]
    for walk in range(n_walks):
        first_step = sample_ball(dim, radius, start)
        heat, path = wos_walk_altered_g(velocity, gradient, start, radius_g, dim, distance_field, min_corner_field, goal, first_step, overall_domain, min_corner, 0)
        if len(path)>0:
            if heat > max_heat:
                max_heat = heat
                path.append(start)
...
def ego_centric_wos_mapping(max_heat, path, domain, min_corner, start, goal, distance_field, min_corner_field, direction, n_walks=1000):
    ...
    domain, new_path, max_heat = WoS_altered_g(0.5, np.array([0,1]), max_heat, distance_field, min_corner_field, goal, start, radius, n_walks, domain, min_corner)
