...
from path import filter_paths, check_path, check_intercept, estimate_gradient, velocity_path
...
def wos_walk_altered_g(velocity, gradient, original, radius_g, dim, distance_field, min_corner_field, goal, start, overall_domain, min_corner, curr_rec):
    if curr_rec == 200:
        return 0, []
    ...
    if np.linalg.norm(gradient)==0:
        dist_original = float('inf')
    else:
        dist_original = np.linalg.norm(start - original)
    if dist_original < dist_b:
        v = start + gradient * dist_b/2
        point, next_steps = wos_walk_altered_g(velocity, gradient, original, radius_g, dim, distance_field, min_corner_field, goal, v, overall_domain, min_corner, curr_rec+1)
        point *= (dist_b/2)/((dist_original+dist_b/2)/2)
...
def ego_centric_wos_mapping(original, max_heat, path, domain, min_corner, start, goal, distance_field, min_corner_field, direction, n_walks=1000):
    radius = min(VIEW_RANGE-5, max(0,dist_to_obstacle(distance_field, min_corner_field, start)-5))
    if not radius:
        return domain, None, [], 0
    
    domain, new_path, max_heat = WoS_altered_g(0.5, np.array([1,1])/np.linalg.norm(np.array([1,1])), original, max_heat, distance_field, min_corner_field, goal, start, radius, n_walks, domain, min_corner)
    ...
    if True:
        next = velocity_path(start, min_corner, radius, domain, n_walks)
        ...
        if len(next) == 1:
            ...
            next_step = next[len(next)//2]

        next_step = start + (next_step-start)/np.linalg.norm(next_step-start)
        if dist_to_obstacle(distance_field, min_corner_field, next_step) > 5.0:
            return domain, next_step, next, 0
        else:
            return domain, None, None, 0
        
    if True:            
        next =  estimate_gradient(start, domain, min_corner, dim, radius/5, n_walks)
...
def path_mapping(start, goal, distance_field, min_corner_field, direction=None, n_walks=1000):
    ...
    while dist_g>distance_field[tuple(np.round(goal-min_corner_field).astype(int))]:
        ...
        solved_domain, start_1, krr, max_heat = ego_centric_wos_mapping(path[0], max_heat, krr, domain, min_corner, start, goal, distance_field, min_corner_field, direction, n_walks)
...
plt.imshow(heatmaps[0], cmap='inferno')
plt.show()
