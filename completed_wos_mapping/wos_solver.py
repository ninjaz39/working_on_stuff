...
class WoSolver:
    def __init__(self, epsilon_shell, max_recursion, avoidance_distance, dim, n_walks):
    ...
    def solve_domain(self, domain, start, goal, radius, radius_g):
        ...
        if radius:
            for walk in range(self.n_walks):
                ...
                heat, path = self.walk(radius_g, domain, goal, first_step)
                if len(path)>0:
                    if heat > max_heat:
                        max_heat = heat
    ...
    def walk(self, radius_g, domain, start, goal, curr_rec=0):
        ...
        if dist_o < dist_b:
            dist_b = dist_o - self.avoidance_distance
            obstacle_hit = True

        if obstacle_hit and dist_b < 0:
            return None, []
        
        if (not obstacle_hit and dist_b<self.epsilon_shell) or self.domain.get_min_heat(start):
            point = 1/dist_g*100000000
            domain.update_heat(start, point)
            return point, [start]
